...
void test_accel_base_constructor() {
    ...
    TEST_ASSERT_TRUE(zero_vector == sensor.get_accel_mps());
    TEST_ASSERT_TRUE(zero_vector == sensor.get_accel_body_mps());
}
...
void test_accel_base_read() {
    ...
    for (int i = 0; i < 3; i++) {
        TEST_ASSERT_FLOAT_WITHIN(1e-10, one_iter[i][0], sensor.get_accel_mps()[i][0]);
        TEST_ASSERT_FLOAT_WITHIN(1e-10, one_iter[i][0], sensor.get_accel_body_mps()[i][0]);
    }
